...
# Store elasticity rules in the database
@app.route('/necos/db/<slice_id>/<part_name>/elasticity_rule', methods=['POST'])
def elasticity_rule(slice_id, part_name):
    rule = request.get_json()
    result, message = SDB.add_elasticity_rule(slice_id, part_name, rule['rule'])
    return jsonify({'result': message}), result

# get elasticity policy
@app.route('/necos/db/<slice_id>/<part_id>/elasticity', methods=['GET'])
def get_elasticity_rule(slice_id, part_id):
    result, message = SDB.get_elasticity_rule(slice_id, part_id)
    if result == 200:
        return jsonify(message), result
    else:
        return abort(result, message)
