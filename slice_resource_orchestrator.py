...
def save_elasticity_rules(content):
    ...
    try:
        for part in content['slices']['sliced']['slice-parts']:
            ...
            if 'elasticity' in part:
                for rule in part['elasticity']:
                    # cal slicesDB to save rule
                    r = requests.post(f'{slices_db_url}/{slice_id}/{part_name}/elasticity_rule', headers = json_header, json = rule)
                    if r.status_code != 201:
                        logger.warning('An elasticity rule could not be saved.')
                    rules += 1
    except:
        logger.info(f'       [X] There were problems when checking for elasticity.')
        return False
    if rules:
        logger.info(f'       - {rules} elasticity policies saved.')
        return True
    else:
