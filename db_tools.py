...
def set_elasticity_parameters_for_json(db_format_policy):
    policy = db_format_policy['policy']
    action = db_format_policy['action']
    for key, value in policy.items():
        if type(value) is str:
            policy[key] = check_for_struct(value)
    for key, value in action.items():
        if type(value) is str:
            action[key] = check_for_struct(value)
    post_deployment = {'post-deployment': action}
    metric_collector = {'metric-collector': {'node-type': policy.pop('metricNodeType'), 'granularity': policy.pop('metricGranularity'), 'metric-name': policy.pop('metricName')}}
